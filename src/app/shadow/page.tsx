...
export default function ShadowPage() {
  ...
  const [logs, setLogs] = useState([
    "[10:45:01] Inicio de sistema completado.",
    "[10:45:03] Inyectando índices de memoria... OK.",
    "[10:45:04] Consultando sensores biométricos... OK.",
    "[10:46:12] Presencia de usuario detectada. Esperando input."
  ]);
  ...
  const handleCommandSubmit = async (e: React.FormEvent) => {
    ...
    setLogs(prev => [...prev, `[SHADOW] Analizando semántica...`]);

    // Call API
    const result = await askShadow(command);
    
    setStatus('speaking');
    setLogs(prev => [...prev, `[SHADOW] ${result.text}`]);
    
    // Calculate reading time based on word count to keep speaking status active
    const wordCount = result.text.split(' ').length;
    const speakingDuration = Math.max(2000, wordCount * 200); // 200ms per word, min 2s

    setTimeout(() => {
      setStatus('idle');
      setLogs(prev => [...prev, `[SHADOW] Tarea completada. Regresando a estado inactivo.`]);
    }, speakingDuration);
  };
  return (
    <div className="w-full h-full flex flex-col pt-8">
      ...
      <motion.div 
        ...
        className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 flex-1"
      >
        
        {/* Main Orb Display (Centerpiece) */}
        <motion.div variants={itemVariants} className="md:col-span-2 lg:col-span-2 row-span-2 bg-[#0a0a0a]/50 border border-[#1a1a1a] rounded-[32px] shadow-2xl relative overflow-hidden group">
        ...
        <motion.div variants={itemVariants} className="md:col-span-1 lg:col-span-2 row-span-2 bg-[#0a0a0a]/50 border border-[#1a1a1a] rounded-[32px] p-6 flex flex-col">
           <div className="flex items-center gap-2 mb-4">
                <Activity className="w-5 h-5 text-gray-500" />
             <h2 className="text-sm font-medium text-[#888]">Flujo Cognitivo</h2>
           </div>
